...
contract XYKswapper is ReentrancyGuard {
    ...
    function addLiquidity(uint256 id, uint256 token0amount, uint256 token1amount) external nonReentrant returns (uint256 liq) {
    ...
    function removeLiquidity(uint256 id, uint256 lp) external nonReentrant returns (uint256 amount0out, uint256 amount1out) {
        IHelios.Pair memory pair = IHelios(msg.sender).pairs(id);
